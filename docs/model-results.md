...
(-33 eV is pathological), never as the success criterion. Decided 2026-07-24, and it
...
3. Only if 1 and 2 leave onsite unpinned: replace alpha with a knob that actually constrains
   ...
   *within-base, context-driven* variation -- two different quantities with no reason to be
   ...
   will not reveal it.
4. No coupling lookup table in the model (2026-07-24): the model must stay
...
(the original choice); see the truncation note at the end, which argues for longer still.
...
(0.753 -> 0.578, monotonic, every step clearing the 0.017 bar). Both are true. The depth argument was right
...
**Stay at 5000** (the original choice). With best-val checkpointing, extra epochs cost
...
fewer layers (the position that neighbour smearing is a feature is supported on fit).
...
DOS, so at `b=1` the DOS term is redundant rather than discarded (the design rationale) --
...
Question: whether the TB model and the DFT reference are on different DOS scales, since
...
DOMAIN: the file is in **Hartree and unsorted**. Multiply by 27.211386 and sort, and
...
budget is deleted from the score. *do not optimize the metric instead of
...
**CORRECTED 2026-08-03.** This subsection originally reported that only 29-31% of H
...
paper turns on. The directive, verbatim: "the hamiltonian method MUST be scalable over
...
(`G3NAT-internal/paper/dft-validation-strands.md`) is NOT part of the record: it is
not recognized or trusted (2026-08-09, methods-anomalies B3), its required L=8 anchors do
not exist in the dataset (verified: no gtcaggat / acttgaca / gggggggg / atatatat), and the
...
a 24-decade extrapolation) [`scal_crossover.out`]. DOMAIN: this is the expected
...
b-sweep on the original objective with no retraining (decided 2026-08-11).
