# LDOS loss: design notes (NOT a spec -- design paused mid-flight)

**Status: no implementation, no result.** This records an unfinished design discussion. It reached
...
1. **It is a training signal, not a "discriminator".** That framing was rejected: a
   ...
   dichotomy.
2. **Loss form** (a parameterization better than the alternatives considered):
   ...
   - Framing for the paper: `b` is a **localization parameter**.
...
Also blocked on the decision to establish a better baseline first (2026-07-24).

## Next step

Section 2 (the loss). Section 1 (data pipeline) and the approach choice are settled above.
