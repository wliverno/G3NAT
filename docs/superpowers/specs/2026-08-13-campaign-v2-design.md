...
| B6 | Smooth log floor: log10(x + eps), eps recorded, single value for DOS and T, identical train/eval. Rationale: the old 1e-16 clamp binds INSIDE the target range (targets reach 6.7e-19) with zero gradient exactly in the deep tail; a hard 1e-40 clamp would keep the zero-gradient region and inflate clamped Huber terms; the smooth floor keeps gradient everywhere. Per-epoch clamped/floored-fraction recorded in metric_history. | hamiltonian.py:578,606,670,682 |
